...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДекорацияСтатус.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Статус на %1: %2';
			|en = 'Статус на %1: %2'"), ДатаПроверкиПодписи, Статус);
...
Асинх Процедура ПослеЧтенияСвойствПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Успех = Неопределено Тогда
		ОшибкаЧтенияПодписи = НСтр("ru = 'Не удалось получить сертификаты из подписи, проверьте настройки программ электронной подписи.';
									|en = 'Не удалось получить сертификаты из подписи, проверьте настройки программ электронной подписи.'");
	Иначе
			...
			Если Строка.ЭтоСертификатПодписи Тогда
				Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Сертификат подписанта: %1';
						|en = 'Сертификат подписанта: %1'"), Строка.КомуВыдан);
			Иначе
				Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Сертификат для проверки подписи: %1';
						|en = 'Сертификат для проверки подписи: %1'"), Строка.КомуВыдан);
			КонецЕсли;
